...
flowchart LR
  subgraph F_TESTS["specs/chuggernaut/tests/"]
    TT["module table_test<br/>32-edge legal + 24-edge illegal samples<br/>(state.rs table_edges, same order)<br/>+ whole-space quantified checks"]
...
than enumerating the function. The 8 legal edges not in that sample:

- 4 Revoked-column edges (Blocked/Ready/Work/Evaluation → Revoked) —
  covered by the quantified `everyNonTerminalMayRevokeTest`;
- Work → Work (row 14), Work → Escalated (row 16), Evaluation → Escalated
  (row 21) — in neither sample list, but emitted by deciders, so every
  firing is audited by `stepRespectsTable`;
- Evaluation → Evaluation (row 17) — in neither sample list and emitted by
  nothing; no test asserts its legality edge-by-edge. It is legal purely by
  the transcribed `(Evaluation, Evaluation | ...)` clause of
  `allowedTransition`.
