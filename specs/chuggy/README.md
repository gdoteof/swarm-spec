...
## The merge-queue + landing PR — one gate, seven proposed requirements (R7 since withdrawn)
...
| **R2** | **One landing per diff.** A ticket's diff lands at most once (decided: charter §2 effect-only exclusivity, proved since PR 1). The group extension was removed with grouping. | Charter §2 exclusivity row (**decided**). |
...
| ~~**R7**~~ | **Withdrawn with grouping** (see above). | — |

### The Batched reading — the note, verbatim, against the mechanics

The note (triage, recorded row): **"Batched from Ready or Blocked"** —
**GROUPING WAS REMOVED** (the grouping-removal PR). PR 5's group half —
`PBatched`, the `BatchMark`s, `decideAbsorb`/`absorbableIn`, the dep
union, the completion fan-out, the dissolution — is gone, and R2's group
extension and R7 with it. Three reasons, in order of weight:

1. **The adopted reading admitted a reachable deadlock.** `absorbableIn`
   refused only a **direct** lead-on-member dep, but the union's drop-out
   rewrites the dep graph at arbitrary distance: with X depping the lead
   and m depping X, absorbing m puts X in the union while X still waits
   on the lead — group → X → group, and neither side ever dispatches. No
   invariant saw it (`depsAcyclic` reads the STORED deps, which
   absorption never rewrites; `stuckSet`'s only base case is
   `PEscalated`, which a pure cycle never reaches, so `deskVisibility`
   held **vacuously** over a permanent deadlock no human task owned).
   Machine-checked before removal — see the finding commit's
   deterministic trace and random probe.
2. **The model was not modeling chuggernaut's batch.** Chuggernaut mints
   a **fresh job** that absorbs members, absorbing them from **Frozen**
   (authoring-side, pre-release), and its composition is the part that
   matters: the members' briefs concatenated into one batch-aware prompt
   and their **evaluator lists unioned BY NAME** (identical duplicates
   dedup, a same-name-different-definition clash is a creation error,
   `require_approval` unions as an OR) — `plan_batch` in chuggernaut's
   `crates/domain/src/decide/authoring.rs`. The absorbed-lead reading had
   none of that: the lead's own program simply stood in for the union, so
   the model carried the grouping's *cost* (a phase, a mark, an
   invariant, a measure rung, a witness module, group-aware visibility
   fixpoints) while modeling none of its *content*.
3. **Nothing needs it.** A batch is expressible as an ordinary ticket
   whose brief covers the work the members would have covered — which is
   what the model always had. Grouping bought no property the machine
   could not state without it.

The same hole exists in chuggernaut itself and is **not** fixed here:
`plan_batch` computes the dep union with no acyclicity check, and
`creates_cycle` (`graph.rs`) DFSes stored dep edges looking for the
candidate, so it structurally cannot see a member→batch completion
relation that is not an edge. Filed as an upstream finding, out of scope
for this model.
...
| ~~R2's group half / R7~~ | **Deleted** (grouping removal): `PBatched` + its rung, `BatchMark`/`Ticket.batch`, `membersOf`/`absorbableIn`/`groupDeps`/`decideAbsorb`, the `absorb` action, `landSuccess`'s fan-out (back to the single transition), the dissolution/park arms in `decideRevoke`, `batchWellFormed`, `landingExclusive`'s mark conjunct, the group edges in the visibility walk (back to single ascending folds), the BATCHING set + its `stepDescends` arm, the batch witness module, and this suite's group tests. The severability claim was tested by exercising it. |
...
grouping exists at all since the removal), and **cross-repo atomic
landings** (nothing lands in two repos atomically). Chuggernaut's **staged gate internals**
...
- **Effect-only exclusivity** (charter §2): any number of task executions
  ...
  (The merge-queue PR's per-group extension, proposed R2, went with the
  grouping removal.)
...
| Merge trains, batching heuristics, cross-repo atomic landings | **Never scheduled — no provenance anywhere, not even proposed** (the merge-queue section): the gate validates one candidate at a time, and grouping was removed outright (a batch is expressible as an ordinary ticket whose brief covers the members' work). |
